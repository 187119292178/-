...
//        System.out.println("用户的id"+userId+jwtEntity);
//        System.out.println("用户的id"+userId+jwtEntity);
//        System.out.println("用户的id"+userId+jwtEntity);
...
//        System.out.println("idwei" +userId);
//        System.out.println("idwei" +userId);
        try {
            // 使用HS256加密算法
            ...
            System.out.println("这是什么"+JwtConstant.USER_ID_KEY);
            System.out.println("这是什么"+ JSON.toJSONString(jwtBuilder));
            // 添加token过期时间

            if(userId.length()>170){
                long TTLMillis2 = jwtEntity.getJwtExpires() * 60 * 1000 * 36;
                System.out.println("多长时间1"+TTLMillis2);
                if (TTLMillis2 >= 0) {
                    long expMillis = nowMillis + TTLMillis2;
                    Date exp = new Date(expMillis);
                    jwtBuilder.setExpiration(exp).setNotBefore(now);
                }
            }else{
                long TTLMillis = jwtEntity.getJwtExpires() * 60 * 1000;
                System.out.println("多长时间2"+TTLMillis);
                if (TTLMillis >= 0) {
    ...
    public static Claims parseToken(String authToken, String base64Secret) {
        try{
            ...
            // TODO 这里自行抛出异常
            logger.error("===== 密钥不匹配 =====", se);
        } catch (ExpiredJwtException ejw) {
            // TODO 这里自行抛出异常
            logger.error("===== token过期 =====", ejw);
        } catch (Exception e){
            // TODO 这里自行抛出异常
